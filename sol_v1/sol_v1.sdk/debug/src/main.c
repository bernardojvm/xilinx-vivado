...
#define PROCESSOR_PBLAZE 0x43C00000 // PICOBLAZE ADDRESS
...
int main (void) {
	...
	print("Enter number of instructions you want to transfer between 1 and 8192\r\n");
	numofbytes =0;
    do{
    ...
    while(test_done==0)
    {
		...
		// reset timer
		cdma_transfer(0,cdma_memory,(u8 *)source,numofbytes);
//		for(int i=0; i<10;i++){
		PBLAZE_mWriteReg(PROCESSOR_PBLAZE, PBLAZE_S_AXI_SLV_REG0_OFFSET, 0x00000001);
		PBLAZE_mWriteReg(PROCESSOR_PBLAZE, PBLAZE_S_AXI_SLV_REG1_OFFSET, 0x30000001);
		PBLAZE_mWriteReg(PROCESSOR_PBLAZE, PBLAZE_S_AXI_SLV_REG2_OFFSET, 0x00000000);
		PBLAZE_mWriteReg(PROCESSOR_PBLAZE, PBLAZE_S_AXI_SLV_REG3_OFFSET, 0x00000001);
		result=PBLAZE_mReadReg(PROCESSOR_PBLAZE, PBLAZE_S_AXI_SLV_REG0_OFFSET);
		xil_printf("Transfered data%d %x \r\n",0,result);
//		}
		while(count<=No_DATA);
		count=0;
		XScuGic_Enable(&InterruptController, 62);
		//cdma_transfer(1,cdma_memory,source,4);
...
void Ready_new_Value(void *baseaddr_p){
	if(count<No_DATA){
	count++;
	PBLAZE_mWriteReg(PROCESSOR_PBLAZE, PBLAZE_S_AXI_SLV_REG0_OFFSET, 0x10000000+count);
	PBLAZE_mWriteReg(PROCESSOR_PBLAZE, PBLAZE_S_AXI_SLV_REG1_OFFSET, 0x30000010);
	PBLAZE_mWriteReg(PROCESSOR_PBLAZE, PBLAZE_S_AXI_SLV_REG2_OFFSET, 0x00000000+count);
	PBLAZE_mWriteReg(PROCESSOR_PBLAZE, PBLAZE_S_AXI_SLV_REG3_OFFSET, 0x00000001);
	//result=PBLAZE_mReadReg(PROCESSOR_PBLAZE, PBLAZE_S_AXI_SLV_REG0_OFFSET);
	//xil_printf("Transfered data%d %x \r\n",count,result);
	}
	...
	count=No_DATA+1;
	result=PBLAZE_mReadReg(PROCESSOR_PBLAZE, PBLAZE_S_AXI_SLV_REG1_OFFSET);
	xil_printf("Transfered data res: %x \r\n",result);
